...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: '',
  // Carpeta MANTENIMIENTOS (en Mi unidad) donde viven "Formatos Mantenimientos
  // Preventivos" y las carpetas de cada cliente. Los informes oficiales se guardan
  // directo en la carpeta del cliente correspondiente (se busca por nombre, o se crea
  // si no existe).
...
async function generarInformeOficial(id){
  ...
  try{
    ...
    await subirArchivoBinario(pdfBlob, nombre + '.pdf', 'application/pdf', folderId);

    setStatus('Guardando copia en Excel…');
    const xlsxBlob = await exportarComoXlsx(sheetFile.id);
    await subirArchivoBinario(xlsxBlob, nombre + '.xlsx', MIME_XLSX, folderId);
