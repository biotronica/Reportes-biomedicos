...
const CONFIG = {
  ...
  // ID de la carpeta de Drive donde se guardarán los informes generados.
  // Déjalo vacío ('') para que se guarden en la raíz de "Mi unidad".
  // El ID es la parte de la URL de la carpeta: drive.google.com/drive/folders/ESTE_ID
  DRIVE_FOLDER_ID: '',
  ...
  const rows = state.equipos.map(eq => `
    <div class="card">
      ...
      <div class="pdf-actions">
        <button class="btn btn-outline btn-sm" style="width:100%;" data-action="verPdf" data-id="${eq.id}">👁️ Ver / Imprimir / Compartir</button>
        <button class="btn btn-outline btn-sm" style="width:100%;" data-action="descargarPdf" data-id="${eq.id}">⬇️ Descargar PDF</button>
        <button class="btn btn-teal btn-sm" style="width:100%;" data-action="subirDrive" data-id="${eq.id}" ${driveOn?'':'disabled style="opacity:.5"'}>☁️ Subir a Google Drive</button>
      </div>
