...
async function revisarSincronizacionRemota(){
  ...
  try{
    ...
    // Si desde otro dispositivo hay una visita más reciente en curso, se toma sola —
    // sin preguntar — para poder continuar exactamente donde quedó. Si no hay nada
    // remoto (o lo remoto no es más nuevo), la pantalla se queda en blanco como
    // siempre, lista para un cliente nuevo.
    if(remotoTieneAlgo && remotoTs > localTs + 5000){
      ...
      saveState();
      goTo('cliente');
      showToast('Se continuó la visita en curso desde otro dispositivo.');
    }
