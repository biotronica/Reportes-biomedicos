...
function marcarOpcion(grid, cambios, labelText){
  const m = findCellsWithText(grid, labelText)[0];
  if(!m) return false;
  marcarCambio(grid, cambios, m.r, m.c + 1, 'X');
  return true;
}
...
function parsearNombreInforme(filename, clienteNombre){
  const m = filename.match(/^Informe_(.+)_(\d{4}-\d{2}-\d{2})\.(pdf|xlsx)$/i);
  if(!m) return null;
  const mitad = m[1];
  const fecha = m[2];
  ...
  let tipoSan = mitad;
  if(mitad.toLowerCase().endsWith('_' + clienteSan.toLowerCase())){
    tipoSan = mitad.slice(0, mitad.length - clienteSan.length - 1);
  }
  const tipoReal = EQUIPOS_LISTA.find(n => sanitizeFilename(n).toLowerCase() === tipoSan.toLowerCase()) || tipoSan.replace(/_/g, ' ');
  return { tipo: tipoReal, fecha, ext };
}
...
async function listarArchivosInforme(carpetaId){
  ...
  const res = await fetch('https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(q) + '&pageSize=1000&fields=files(id,name,mimeType)', { headers: driveHeaders() });
  if(!res.ok) return [];
  const data = await res.json();
  return data.files || [];
}
...
async function importarDesdeCarpetasExistentes(onProgreso){
  ...
  for(const carpeta of carpetas){
    ...
    for(const base in grupos){
      ...
      nuevasFilas.push([parsed.fecha, carpeta.name, parsed.tipo, '', excelUrl, pdfUrl]);
...
async function registrarVisitaCliente(cliente, equipo, links){
  ...
  const filaValores = [cliente.nombre, cliente.solicitante, cliente.personaCargo, cliente.contacto, cliente.fecha];
...
async function prepararGuardado(id){
  ...
  confirmacionCarpeta[id] = { cargando: true, opciones: null, elegida: null, buscando: false, filtro: '' };
  render();
  try{
    const folderId = await encontrarOCrearCarpetaCliente(state.cliente.nombre);
    const nombre = await obtenerNombreCarpeta(folderId);
    confirmacionCarpeta[id] = { cargando: false, opciones: null, elegida: { id: folderId, nombre }, buscando: false, filtro: '' };
  }catch(err){
    confirmacionCarpeta[id] = { cargando: false, opciones: null, elegida: null, buscando: false, filtro: '', error: err.message };
  }
  render();
}
...
async function mostrarSelectorCarpetas(id){
  ...
  if(!est.opciones){
    try{
      est.opciones = await listarCarpetasDeMantenimientos();
    }catch(err){
...
function panelConfirmacionHtml(eq){
  ...
  const listaHtml = est.buscando
    ? `<div class="field"><input type="text" id="carpetaFiltro_${eq.id}" placeholder="Buscar carpeta…" value="${escapeHtml(est.filtro||'')}"></div>
...
function filasCarpetasHtml(equipoId, filtro){
  ...
  const lista = est.opciones
    ...
    .slice(0, 50);
  if(!lista.length) return '<div class="status-line">Sin resultados.</div>';
  return lista.map(c => `
    <div class="equipo-card" data-action="elegirCarpetaGuardado" data-id="${equipoId}" data-folderid="${c.id}" data-foldername="${escapeHtml(c.name)}">
      <div class="equipo-icon">📁</div>
      <div class="equipo-info"><div class="equipo-nombre">${escapeHtml(c.name)}</div></div>
    </div>`).join('');
}
