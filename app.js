...
function pickClienteExistente(nombre){
  if(state.equipos.length && state.cliente.nombre && state.cliente.nombre !== nombre){
    if(!confirm('Ya tienes ' + state.equipos.length + ' equipo(s) agregado(s) para "' + state.cliente.nombre + '". Al cambiar de cliente se van a quitar de esta visita (no se borra nada de lo ya guardado en Drive). ¿Continuar?')) return;
    state.equipos = [];
    state.draft = null;
    state.editingId = null;
  }
