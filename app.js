...
function botonActaEntregaHtml(){
  ...
  return `
    <div class="card" style="background:#f3eefc;">
      ...
      <div class="status-line" style="text-align:left;margin:0 0 10px;">
        Genera un Word con la tabla de los ${state.equipos.length} equipo(s) de esta visita, agrupados por tipo.
      </div>
      <button class="btn btn-primary btn-sm" style="width:100%;" data-action="generarActaEntrega">📄 Generar acta de entrega (Word)</button>
...
async function generarActaEntrega(){
  ...
  try{
    setStatus('Generando…');
    const { Document, Paragraph, Table, TableRow, TableCell, TextRun, Packer, HeadingLevel, WidthType, AlignmentType, BorderStyle } = docx;
    ...
    const anchosPorcentaje = [10, 20, 30, 20, 20];
    const encabezados = ['Ítem', 'Número reporte', 'Equipo', 'Serie', 'Inventario'];

    function celda(texto, esEncabezado, anchoPct){
      return new TableCell({
        children: [new Paragraph({
          alignment: esEncabezado ? AlignmentType.CENTER : AlignmentType.LEFT,
          children: [new TextRun({ text: String(texto || ''), bold: !!esEncabezado })]
        })],
        width: { size: anchoPct, type: WidthType.PERCENTAGE },
        borders: bordes,
        shading: esEncabezado ? { fill: 'D9D9D9' } : undefined
      });
    }

    const filaEncabezado = new TableRow({
      tableHeader: true,
      children: encabezados.map((t, i) => celda(t, true, anchosPorcentaje[i]))
    });

    const filasDatos = equiposAgrupadosParaActa().map(f => new TableRow({
      children: [
        celda(f.item, false, anchosPorcentaje[0]),
        celda(f.informeNo, false, anchosPorcentaje[1]),
        celda(f.tipo, false, anchosPorcentaje[2]),
        celda(f.serie, false, anchosPorcentaje[3]),
        celda(f.codigo, false, anchosPorcentaje[4])
      ]
    }));
    ...
    const doc = new Document({
      sections: [{
        properties: {},
        children: [
          new Paragraph({ text: 'ACTA DE ENTREGA', heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER }),
          new Paragraph({ text: '' }),
          new Paragraph({ children: [new TextRun({ text: 'Cliente: ', bold: true }), new TextRun(state.cliente.nombre || '')] }),
          new Paragraph({ children: [new TextRun({ text: 'Fecha: ', bold: true }), new TextRun(state.cliente.fecha || '')] }),
          new Paragraph({ text: '' }),
          tabla
        ]
      }]
    ...
    // Además, si hay Drive conectado, se sube una copia a la carpeta del cliente
    if(state.driveToken){
      try{
...
function onAppClick(e){
  ...
  switch(action){
    ...
    case 'generarActaEntrega': generarActaEntrega(); break;
