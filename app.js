...
function loadState(){
  try{
    ...
    // Cada vez que se abre la app debe quedar lista para un cliente nuevo, sin
    // arrastrar los datos de la última visita — por eso cliente y equipos NUNCA se
    // restauran aquí, aunque hayan quedado guardados. Lo demás (si algún día se agrega
    // algo que sí deba persistir entre sesiones) sí seguiría restaurándose normalmente.
    parsed.cliente = defaultCliente();
    parsed.equipos = [];
    parsed.draft = null;
    parsed.editingId = null;
    parsed.screen = 'cliente';
    parsed.actaEligiendoSaludo = false;
    // OJO: aquí NO se toca _localUpdatedAt. Si se pusiera en el momento actual, la
    // pantalla en blanco siempre parecería "más nueva" que el trabajo real guardado
    // hace un momento, y la sincronización automática (revisarSincronizacionRemota)
    // nunca detectaría que hay una visita en curso para continuar.
    return parsed;
  }catch(e){ return null; }
}
...
function appendDictatedText(path, targetId, text){
  ...
  let updated;
  if(path === 'draft.informeNo'){
    // El número se dicta por partes ("90", "37", "guion", "26"), en una sola vez o
    // tocando el micrófono varias veces: todo queda junto, p. ej. 9037-26.
    updated = normalizarInformeDictado(current + ' ' + text);
  }else if(path === 'draft.serie' || path === 'draft.codigo'){
    // Cifras de dos en dos, letras y guiones: se pega a lo que ya había, sin espacios
    // (lo que ya estaba escrito no se toca, por si tiene barras u otros signos).
    updated = current.trim() + normalizarSerieDictada(text);
  }else{
    if(path === 'cliente.nombre') text = abreviarTitulosDictados(text);
    updated = current.trim() ? (current.trim() + ' ' + text) : text;
  ...
  saveState();
  if(path === 'cliente.nombre' && typeof renderClienteSuggestions === 'function') renderClienteSuggestions(updated);
}

// "doctora" -> "Dra.", "doctor" -> "Dr." (solo palabras completas)
function abreviarTitulosDictados(texto){
  return String(texto || '')
    .replace(/\bdoctora\b\.?/gi, 'Dra.')
    .replace(/\bdoctor\b\.?/gi, 'Dr.');
}

// Convierte lo dictado para "Informe No." en solo cifras y guion, todo junto.
// Acepta cifras ("90 37 guion 26") y también números en palabras
// ("noventa treinta y siete guion veintiséis") por si el dictado no los pasa a cifras.
function normalizarInformeDictado(texto){
  return normalizarCodigoDictado(texto, false);
}

// Serie y código interno: cifras dichas de dos en dos, letras sueltas ("a", "be",
// "equis"...) y "guion" — todo queda junto y en mayúsculas, p. ej. "AB-1234".
function normalizarSerieDictada(texto){
  return normalizarCodigoDictado(texto, true);
}

function normalizarCodigoDictado(texto, conLetras){
  const original = String(texto || '');
  const conGuion = original
    .replace(/(\d)[.,](?=\d)/g, '$1')            // "9.037" o "9,037" -> "9037"
    .replace(/[-\u2010-\u2015]/g, ' - ')
    .replace(/\//g, ' / ');
  const UNIDADES = {cero:0,un:1,uno:1,una:1,dos:2,tres:3,cuatro:4,cinco:5,seis:6,siete:7,ocho:8,nueve:9};
  const ESPECIALES = {diez:10,once:11,doce:12,trece:13,catorce:14,quince:15,dieciseis:16,diecisiete:17,dieciocho:18,diecinueve:19,
  ...
  const CENTENAS = {cien:100,ciento:100,doscientos:200,trescientos:300,cuatrocientos:400,quinientos:500,seiscientos:600,setecientos:700,ochocientos:800,novecientos:900};
  // Nombre de cada letra en español (y variantes comunes del dictado)
  const LETRAS = {a:'A',be:'B',ce:'C',de:'D',e:'E',efe:'F',ge:'G',hache:'H',i:'I',jota:'J',ka:'K',ca:'K',ele:'L',eme:'M',ene:'N',
    'eñe':'Ñ',o:'O',pe:'P',cu:'Q',erre:'R',ere:'R',ese:'S',te:'T',u:'U',uve:'V',ve:'V',equis:'X',ye:'Y',zeta:'Z',ceta:'Z'};

  let salida = '', actual = null, despuesDeY = false;
  const cerrar = () => { if(actual !== null){ salida += String(actual); actual = null; } despuesDeY = false; };
  const tokens = conGuion.split(/\s+/).filter(Boolean);
  for(let i = 0; i < tokens.length; i++){
    const crudo = tokens[i];
    const tok = crudo.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.,;:]+$/, '');
    const tokConEnie = crudo.toLowerCase().replace(/[.,;:]+$/, '');
    const siguiente = (tokens[i+1] || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    if(!tok) continue;
    if(tok === '-' || tok === 'guion' || tok === 'gion' || tok === 'raya' || tok === 'menos'){ cerrar(); salida += '-'; continue; }
    if(conLetras && (tok === '/' || tok === 'barra' || tok === 'slash')){ cerrar(); salida += '/'; continue; }
    if(conLetras && tok === 'punto'){ cerrar(); salida += '.'; continue; }
    if(/\d/.test(tok)){ cerrar(); salida += conLetras ? tok.replace(/[^a-z0-9]/g, '').toUpperCase() : tok.replace(/\D/g, ''); continue; }
    if(tok === 'y'){
      // "treinta y siete" = número; si no, en serie es la letra Y
      if(actual !== null && actual % 10 === 0 && actual % 100 >= 30){ despuesDeY = true; continue; }
      if(conLetras){ cerrar(); salida += 'Y'; }
      continue;
    }
    if(tok in CENTENAS){ cerrar(); actual = CENTENAS[tok]; continue; }
    if(tok in DECENAS || tok in ESPECIALES){
      const v = DECENAS[tok] !== undefined ? DECENAS[tok] : ESPECIALES[tok];
      if(actual !== null && actual % 100 !== 0) cerrar();
      actual = (actual || 0) + v; despuesDeY = false; continue;
    }
    if(tok in UNIDADES){
      const v = UNIDADES[tok];
      const sumaAlGrupo = actual !== null && (
        actual % 100 === 0 ||                                      // "ciento tres"
        (despuesDeY && actual % 10 === 0 && actual % 100 >= 30)    // "treinta y siete"
      );
      if(!sumaAlGrupo) cerrar();
      actual = sumaAlGrupo ? actual + v : v;
      despuesDeY = false; continue;
    }
    if(!conLetras) continue; // en Informe No. cualquier otra palabra se ignora
    // Letras: "doble ve"/"doble u" = W, "i griega" = Y
    if(tok === 'doble' && (siguiente === 've' || siguiente === 'u' || siguiente === 'uve' || siguiente === 'v')){ cerrar(); salida += 'W'; i++; continue; }
    if(tok === 'i' && siguiente === 'griega'){ cerrar(); salida += 'Y'; i++; continue; }
    if(tokConEnie in LETRAS){ cerrar(); salida += LETRAS[tokConEnie]; continue; }
    if(tok in LETRAS){ cerrar(); salida += LETRAS[tok]; continue; }
    if(/^[a-zñ]$/.test(tokConEnie)){ cerrar(); salida += tokConEnie.toUpperCase(); continue; }
    // Siglas que el dictado ya entrega en mayúsculas ("SN", "NI") se conservan
    if(/^[A-ZÑ]{1,6}$/.test(crudo.replace(/[.,;:]+$/, ''))){ cerrar(); salida += crudo.replace(/[.,;:]+$/, ''); continue; }
    // cualquier otra palabra ("número", "serie"...) se ignora
  }
  cerrar();
...
function saveDraft(){
  ...
  ['marca','modelo','ubicacion','responsable','recibeSatisfaccion'].forEach(k => guardarEnHistorial('draft.' + k, d[k]));
  if(state.editingId){
...
function clienteSelectorHtml(c){
  if(!state.driveToken){
    return fieldMicHtml('cliente.nombre', c.nombre, 'Nombre del cliente', false);
  }
  if(!clientesCache){
    cargarBaseClientes().then(render).catch(()=>{});
  }
  return fieldMicHtml('cliente.nombre', c.nombre, 'Cliente (escribe para buscar o crear uno nuevo)', false)
...
function renderClienteSuggestions(query){
  ...
  if(!box) return;
  if(!clientesCache){ box.innerHTML = ''; box.className = ''; return; }
  const q = normNombre(query || '');
  const matches = clientesCache.clientes.filter(x => q && normNombre(x.nombre).includes(q));
  if(!q || !matches.length){ box.innerHTML = ''; box.className = ''; return; }
  box.className = 'equipo-suggestions';
  box.innerHTML = matches.slice(0, 20)
    .map(x => `<div class="equipo-suggestion" data-action="pickClienteExistente" data-value="${escapeHtml(x.nombre)}">${escapeHtml(x.nombre)}</div>`)
    .join('');
}

// Se llama cada vez que el nombre del cliente cambia de verdad (no solo se corrige un
// typo del mismo). Si ya hay equipos agregados para el cliente anterior, los borra de
// esta visita (nunca borra nada ya guardado en Drive). Devuelve false si el usuario
// cancela, para que quien llama pueda deshacer el cambio de nombre.
function confirmarCambioClienteSiHaceFalta(nombreAnterior, nombreNuevo){
  nombreAnterior = (nombreAnterior || '').trim();
  nombreNuevo = (nombreNuevo || '').trim();
  if(!state.equipos.length) return true;
  if(normNombre(nombreAnterior) === normNombre(nombreNuevo)) return true;
  if(!nombreAnterior) return true; // no había cliente antes, nada que borrar
  if(!confirm('Ya tienes ' + state.equipos.length + ' equipo(s) agregado(s) para "' + nombreAnterior + '". Al cambiar de cliente se van a quitar de esta visita (no se borra nada de lo ya guardado en Drive). ¿Continuar?')){
    return false;
  }
  state.equipos = [];
  state.draft = null;
...
function pickClienteExistente(nombre){
  if(!confirmarCambioClienteSiHaceFalta(state.cliente.nombre, nombre)) return;
  state.cliente.nombre = nombre;
  state.cliente.solicitante = '';
  state.cliente.personaCargo = '';
  state.cliente.contacto = '';
  manejarNombreClienteIngresado(nombre);
  saveState();
  render();
}
...
function fieldMicHtml(path, value, label, isTextarea, targetIdOverride){
  ...
  const inputHtml = isTextarea
    ...
    : `<input type="text" id="${targetId}" data-path="${path}" value="${escapeHtml(value)}" placeholder="${escapeHtml(label)}"${CAMPOS_CON_HISTORIAL.includes(path) ? ' data-historial="1" autocomplete="off"' : ''}>`;
  const sugHtml = (!isTextarea && CAMPOS_CON_HISTORIAL.includes(path)) ? `<div id="hist_${targetId}"></div>` : '';
  // equipo-search = position:relative, para que la lista de sugerencias salga justo debajo del campo
  return `
    <div class="field${sugHtml ? ' equipo-search' : ''}">
...
/* ============== AUTOCOMPLETAR CON LO YA ESCRITO ==============
   El autocompletado del navegador en el PC (Chrome/Edge) sale de su propio historial de
   formularios; Safari en iPhone no guarda ese historial para campos comunes, así que allá
   no aparecía nada. Esta lista propia funciona igual en los dos: cada valor que se escribe
   en estos campos se guarda en el dispositivo y se sugiere al teclear las primeras letras. */
const CAMPOS_CON_HISTORIAL = [
  'draft.marca', 'draft.modelo', 'draft.ubicacion', 'draft.responsable', 'draft.recibeSatisfaccion',
  'cliente.solicitante', 'cliente.personaCargo', 'cliente.contacto'
];
const HISTORIAL_KEY = 'reportes_historial_campos_v1';
const HISTORIAL_MAX = 60;

function leerHistorial(){
  try{ return JSON.parse(localStorage.getItem(HISTORIAL_KEY)) || {}; }catch(e){ return {}; }
}
function guardarEnHistorial(path, valor, historial){
  const v = String(valor || '').trim();
  if(v.length < 2 || !CAMPOS_CON_HISTORIAL.includes(path)) return;
  const h = historial || leerHistorial();
  const lista = (h[path] || []).filter(x => normNombre(x) !== normNombre(v));
  lista.unshift(v); // lo más reciente primero
  h[path] = lista.slice(0, HISTORIAL_MAX);
  if(!historial){ try{ localStorage.setItem(HISTORIAL_KEY, JSON.stringify(h)); }catch(e){} }
}
// La primera vez se llena con lo que ya hay en la visita actual, para no empezar vacío.
function sembrarHistorialSiHaceFalta(){
  if(localStorage.getItem(HISTORIAL_KEY)) return;
  const h = {};
  (state.equipos || []).forEach(eq => {
    ['marca','modelo','ubicacion','responsable','recibeSatisfaccion'].forEach(k => guardarEnHistorial('draft.' + k, eq[k], h));
  });
  ['solicitante','personaCargo','contacto'].forEach(k => guardarEnHistorial('cliente.' + k, state.cliente && state.cliente[k], h));
  try{ localStorage.setItem(HISTORIAL_KEY, JSON.stringify(h)); }catch(e){}
}
function renderHistorialSugerencias(input){
  const box = document.getElementById('hist_' + input.id);
  if(!box) return;
  const q = normNombre(input.value);
  const lista = leerHistorial()[input.dataset.path] || [];
  let matches = [];
  if(q){
    const empiezan = lista.filter(x => normNombre(x).startsWith(q));
    const contienen = lista.filter(x => !normNombre(x).startsWith(q) && normNombre(x).includes(q));
    matches = empiezan.concat(contienen).filter(x => normNombre(x) !== q).slice(0, 6);
  }
  if(!matches.length){ box.innerHTML = ''; box.className = ''; return; }
  box.className = 'equipo-suggestions';
  box.innerHTML = matches.map(x =>
    `<div class="equipo-suggestion" data-action="pickHistorial" data-target="${escapeHtml(input.id)}" data-value="${escapeHtml(x)}">${escapeHtml(x)}</div>`
  ).join('');
}
function pickHistorial(targetId, valor){
  const input = document.getElementById(targetId);
  if(!input) return;
  input.value = valor;
  setPath(input.dataset.path, valor);
  guardarEnHistorial(input.dataset.path, valor);
  saveState();
  const box = document.getElementById('hist_' + targetId);
...
function renderExportar(){
  ...
  const rows = state.equipos.map(eq => {
    ...
    const yaGenerado = eq.informeOficialGenerado;
    return `
    <div class="card">
      <div class="card-title" style="display:flex;align-items:center;gap:8px;justify-content:space-between;">
        ...
        ${yaGenerado ? `<span class="badge-listo" title="Generado el ${escapeHtml(eq.informeOficialFecha||'')}">✓ Ya generado</span>` : ''}
      </div>
      ...
      <div class="pdf-actions">
        ${tieneTemplate ? `<button class="btn ${yaGenerado?'btn-outline':'btn-primary'} btn-sm" style="width:100%;" data-action="prepararGuardado" data-id="${eq.id}" ${driveOn?'':'disabled style="opacity:.5"'}>📄 ${yaGenerado ? 'Generar de nuevo (formato real)' : 'Generar informe oficial (formato real)'}</button>` : `<div class="status-line">Sin plantilla oficial para este equipo — usa el PDF genérico abajo.</div>`}
  ...
  <div>
    ...
    <div class="content">
      ...
      ${botonMasivoHtml()}
      ${rows}
      ${botonActaEntregaHtml()}
      <button class="btn btn-ghost" data-action="iniciarOtroCliente" style="margin-top:6px;">+ Iniciar informe para otro cliente</button>
    </div>
  </div>`);
  return wrap;
}

const ID_MASIVO = '__todos__';

function equiposPendientes(){
  return state.equipos.filter(eq => TEMPLATE_FILE_IDS[eq.tipo] && !eq.informeOficialGenerado);
}

function botonMasivoHtml(){
  const pendientes = equiposPendientes();
  if(state.equipos.length < 2) return ''; // no tiene sentido con 0 o 1 equipo
  const driveOn = !!state.driveToken && Date.now() < state.driveExpiry;
  const panel = panelConfirmacionHtml({ id: ID_MASIVO }, true);
  return `
    <div class="card" style="background:#eef5ff;">
      <div class="card-title">📄 Generar todos los informes oficiales</div>
      <div class="status-line" style="text-align:left;margin:0 0 10px;">
        ${pendientes.length ? `Quedan ${pendientes.length} equipo(s) pendiente(s) por generar.` : 'Ya generaste el informe oficial de todos los equipos con plantilla.'}
      </div>
      <button class="btn btn-primary btn-sm" style="width:100%;" data-action="prepararGuardadoMasivo" ${(driveOn && pendientes.length) ? '' : 'disabled style="opacity:.5"'}>📄 Generar todos los pendientes (${pendientes.length})</button>
      ${panel}
      <div class="status-line" id="status_${ID_MASIVO}"></div>
    </div>`;
}

/* ============== ACTA DE ENTREGA (Word, usando la plantilla real de Intelmedica) ============== */
function botonActaEntregaHtml(){
  if(!state.equipos.length) return '';
  if(state.actaEligiendoSaludo){
    return `
    <div class="card" style="background:#f3eefc;">
      <div class="card-title">📄 Acta de entrega</div>
      <div class="status-line" style="text-align:left;margin:0 0 10px;">
        ¿Cómo se debe encabezar el acta para <b>${escapeHtml(state.cliente.nombre || 'este cliente')}</b>?
      </div>
      <button class="btn btn-outline btn-sm" style="width:100%;margin-bottom:6px;" data-action="generarActaEntrega" data-saludo="empresa">🏢 Empresa (Señores)</button>
      <button class="btn btn-outline btn-sm" style="width:100%;margin-bottom:6px;" data-action="generarActaEntrega" data-saludo="hombre">🧑 Hombre (Señor)</button>
      <button class="btn btn-outline btn-sm" style="width:100%;margin-bottom:6px;" data-action="generarActaEntrega" data-saludo="mujer">👩 Mujer (Señora)</button>
      <button class="btn btn-outline btn-sm" style="width:100%;margin-bottom:6px;" data-action="generarActaEntrega" data-saludo="doctor">🩺 Dr. (Doctor)</button>
      <button class="btn btn-outline btn-sm" style="width:100%;margin-bottom:6px;" data-action="generarActaEntrega" data-saludo="doctora">🩺 Dra. (Doctora)</button>
      <button class="btn btn-ghost btn-sm" style="width:100%;" data-action="cancelarActaEntrega">Cancelar</button>
    </div>`;
  }
  return `
    <div class="card" style="background:#f3eefc;">
      <div class="card-title">📄 Acta de entrega</div>
      <div class="status-line" style="text-align:left;margin:0 0 10px;">
        Genera el acta (Word) con la tabla de los ${state.equipos.length} equipo(s) de esta visita, agrupados por tipo.
      </div>
      <button class="btn btn-primary btn-sm" style="width:100%;" data-action="pedirSaludoActa">📄 Generar acta de entrega (Word)</button>
      <div class="status-line" id="status_acta"></div>
    </div>`;
}

function pedirSaludoActa(){
  state.actaEligiendoSaludo = true;
  render();
}

function cancelarActaEntrega(){
  state.actaEligiendoSaludo = false;
  render();
}

// Agrupa los equipos por tipo, ordena los grupos alfabéticamente (ascendente) y
// numera los ítems de forma continua a lo largo de toda la tabla.
function equiposAgrupadosParaActa(){
  const grupos = {};
  state.equipos.forEach(eq => {
    const clave = eq.tipo || 'Sin tipo';
    if(!grupos[clave]) grupos[clave] = [];
    grupos[clave].push(eq);
  });
  const tiposOrdenados = Object.keys(grupos).sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
  const filas = [];
  let item = 1;
  tiposOrdenados.forEach(tipo => {
    grupos[tipo].forEach(eq => {
      filas.push({ item: item++, informeNo: eq.informeNo || '', tipo, serie: eq.serie || '', codigo: eq.codigo || '' });
    });
  });
  return filas;
}
...
function celdaActaXml(texto, esEncabezado, anchoTwips, centrado){
  ...
  const vAlign = esEncabezado ? '<w:vAlign w:val="center"/>' : '';
  const lineas = String(texto == null ? '' : texto).split('\n');
  const contenido = lineas.map((linea, i) =>
    (i > 0 ? '<w:br/>' : '') + '<w:t xml:space="preserve">' + escapeXml(linea) + '</w:t>'
  ).join('');
  return '<w:tc><w:tcPr><w:tcW w:w="' + anchoTwips + '" w:type="dxa"/>' + sombreado + vAlign + '</w:tcPr>'
    + '<w:p><w:pPr><w:spacing w:after="0"/>' + alineacion + '<w:rPr><w:sz w:val="20"/></w:rPr></w:pPr>'
    + '<w:r><w:rPr>' + negrilla + '<w:sz w:val="20"/></w:rPr>' + contenido + '</w:r></w:p></w:tc>';
}
...
function anchoColumnaActa(textos){
  const lineas = textos.flatMap(t => String(t || '').split('\n'));
  const masLargo = lineas.reduce((max, t) => Math.max(max, t.length), 0);
...
// Quita títulos al inicio del nombre (Dr., Dra., Doctor, Doctora, Sr., Sra., Señor, Señora)
// para que en el acta el nombre quede solo, ya que el título va en la línea del saludo.
function nombreSinTitulo(nombre){
  return String(nombre || '').replace(/^\s*(?:(?:dra|dr|doctora|doctor|sra|sr|señora|señor)\b\.?\s*)+/i, '').trim();
}

// Saca de la plantilla Word la imagen de fondo del membrete (la del encabezado)
// y la pasa a JPEG liviano para ponerla de fondo en el PDF.
async function membreteActaDataUrl(zip){
  let ruta = 'word/media/image2.png';
  try{
    const rels = await zip.file('word/_rels/header1.xml.rels').async('string');
    const m = rels.match(/Target="(media\/[^"]+)"/);
    if(m) ruta = 'word/' + m[1];
  }catch(e){ /* se usa la ruta por defecto */ }
  const archivo = zip.file(ruta);
  if(!archivo) return null;
  const blobImg = await archivo.async('blob');
  const url = URL.createObjectURL(blobImg);
  try{
    const img = await new Promise((resolve, reject) => {
      const i = new Image();
      i.onload = () => resolve(i);
      i.onerror = () => reject(new Error('No se pudo leer el membrete'));
      i.src = url;
    });
    const canvas = document.createElement('canvas');
    canvas.width = 1275; canvas.height = 1650; // mitad de resolución: se ve bien y pesa poco
...
function generarActaPdfBlob(saludoTexto, membreteDataUrl){
  ...
  if(membreteDataUrl){
    // Misma posición y tamaño que en el Word: 205.3 x 265.5 mm, alineado a la derecha, arriba.
    const anchoMembrete = 205.3, altoMembrete = 265.5;
    doc.addImage(membreteDataUrl, 'JPEG', pageW - anchoMembrete, 0.3, anchoMembrete, altoMembrete);
    y = 52; // debajo de la onda y el logo del membrete
  }else{
    y = 20;
  ...
  doc.text(nombreSinTitulo(state.cliente.nombre), marginX, y);
  y += 10;
  ...
  const encabezados = ['Ítem', 'Numero\nreporte', 'Equipo', 'Serie', 'Inventario'];
  const claves = ['item', 'informeNo', 'tipo', 'serie', 'codigo'];
  const CENTRADAS = ['item', 'informeNo', 'codigo'];
  const filasDatos = equiposAgrupadosParaActa();
  const MM_POR_CARACTER = 2.1, RELLENO_MM = 4;
  const anchosMm = claves.map((clave, i) => {
    const lineas = [encabezados[i], ...filasDatos.map(f => String(f[clave] || ''))].flatMap(t => String(t).split('\n'));
    const masLargo = lineas.reduce((m, t) => Math.max(m, t.length), 0);
  ...
  const alturaFila = 7;
  const alturaFilaEncabezado = 11;

  function filaTabla(valores, esEncabezado){
    let x = xInicial;
    const alto = esEncabezado ? alturaFilaEncabezado : alturaFila;
    doc.setFont(undefined, esEncabezado ? 'bold' : 'normal');
    doc.setFontSize(9);
    if(esEncabezado){ doc.setFillColor(217,217,217); doc.rect(xInicial, y, anchoTablaTotal, alto, 'F'); }
    anchosMm.forEach((ancho, i) => {
      const lineas = String(valores[i] == null ? '' : valores[i]).split('\n');
      const centrar = esEncabezado || CENTRADAS.includes(claves[i]);
      const tx = centrar ? x + ancho/2 : x + 1.5;
      const yTextoInicial = esEncabezado
        ? (lineas.length > 1 ? y + 4.3 : y + alto/2 + 1.5)
        : y + alto - 2.3;
      lineas.forEach((linea, li) => {
        doc.text(linea, tx, yTextoInicial + li * 4, centrar ? { align: 'center' } : undefined);
      });
...
async function generarActaEntrega(saludoElegido){
  state.actaEligiendoSaludo = false;
  render();
  const elStatus = document.getElementById('status_acta');
  const setStatus = (msg, cls) => { if(elStatus){ elStatus.textContent = msg; elStatus.className = 'status-line ' + (cls||''); } };
  try{
    setStatus('Generando…');

    const res = await fetch('plantilla-acta.docx');
    if(!res.ok) throw new Error('No se pudo cargar la plantilla del acta (HTTP ' + res.status + ')');
    const bytesPlantilla = await res.arrayBuffer();
    const zip = await JSZip.loadAsync(bytesPlantilla);
    let xml = await zip.file('word/document.xml').async('string');

    // 1) Fecha, justo después de "Santiago de Cali,"
    const viejoFecha = '<w:t>Santiago de Cali</w:t></w:r><w:r w:rsidR="00446343"><w:rPr><w:i/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr><w:t>,</w:t></w:r><w:r w:rsidR="00A631EF"><w:rPr><w:i/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr><w:t xml:space="preserve"> </w:t></w:r>';
    const nuevoFecha = '<w:t>Santiago de Cali</w:t></w:r><w:r w:rsidR="00446343"><w:rPr><w:i/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr><w:t>, ' + escapeXml(fechaLarga(state.cliente.fecha)) + '</w:t></w:r>';
    if(xml.includes(viejoFecha)) xml = xml.replace(viejoFecha, nuevoFecha);

    // 2) Saludo, según lo elegido
    const SALUDOS = { empresa: 'Señores', hombre: 'Señor', mujer: 'Señora', doctor: 'Doctor', doctora: 'Doctora' };
    const saludoTexto = SALUDOS[saludoElegido] || 'Señores';
    xml = xml.replace('<w:t>Señor</w:t>', '<w:t>' + escapeXml(saludoTexto) + '</w:t>');

    // 3) Nombre del cliente en vez de la raya
    xml = xml.replace('<w:t>_______________________________</w:t>', '<w:t>' + escapeXml(nombreSinTitulo(state.cliente.nombre)) + '</w:t>');

    // 4) Tabla de equipos, reemplazando los renglones en blanco que trae la plantilla
    //    para no correr la firma a una segunda página.
    const marcadorInicio = 'siguientes equipos</w:t></w:r><w:r w:rsidR="009C602E"><w:rPr><w:i/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr><w:t>:</w:t></w:r></w:p>';
    const idxInicio = xml.indexOf(marcadorInicio);
    const idxCordialmente = xml.indexOf('<w:t>Cordialmente,</w:t>');
    if(idxInicio !== -1 && idxCordialmente !== -1){
      const idxInicioTabla = idxInicio + marcadorInicio.length;
      const idxFinParrafo = xml.lastIndexOf('<w:p ', idxCordialmente);
      ...
      const filasDatosXml = filasDatos.map(f => '<w:tr>' + claves.map((clave, i) => celdaActaXml(f[clave], false, anchos[i], CENTRADAS.includes(clave))).join('') + '</w:tr>').join('');
      ...
      xml = xml.slice(0, idxInicioTabla) + tablaXml + xml.slice(idxFinParrafo);
    }
    ...
    if(state.driveToken){
      ...
      try{
        ...
        let membrete = null;
        try{ membrete = await membreteActaDataUrl(zip); }catch(e){ membrete = null; }
        const pdfBlobActa = generarActaPdfBlob(saludoTexto, membrete);
...
function iniciarOtroCliente(){
  if(state.equipos.length){
    if(!confirm('Ya tienes ' + state.equipos.length + ' equipo(s) agregado(s) para "' + (state.cliente.nombre||'este cliente') + '". Al iniciar con otro cliente se van a quitar de esta visita (no se borra nada de lo ya guardado en Drive). ¿Continuar?')) return;
  }
  state.cliente = defaultCliente();
  state.equipos = [];
  state.draft = null;
  state.editingId = null;
  saveState();
...
function nombreArchivo(equipo){
  ...
  const marca = sanitizeFilename(equipo.marca || 'SM');
  const serie = sanitizeFilename(equipo.serie || 'SN');
  return `${numero}_${tipo}_${marca}_${serie}.pdf`;
}
...
// Estas posiciones son exactas para la plantilla V7 (misma para los 74 equipos, porque
// todos comparten el mismo encabezado). Coordenadas 0-indexadas (fila, columna)
// verificadas una por una contra el archivo real con una prueba visual — por eso ya no
// se buscan por texto ni por un cálculo aproximado: así no quedan fuera de su cuadro ni
// en la celda equivocada.
const CASILLAS_V7 = {
  Correctivo: [13, 2], Preventivo: [16, 2],
  Predictivo: [13, 11], Diagnóstico: [16, 11],
  Mecánica: [13, 18], Eléctrica: [16, 18],
  Electrónica: [13, 25], OtroFalla: [16, 25],
  FueraServicioSI: [28, 22], FueraServicioNO: [28, 25],
  InteriorSI: [34, 7], InteriorNO: [34, 11],
  ExteriorSI: [34, 22], ExteriorNO: [34, 25]
};

// Igual de exactos: dónde va cada dato de texto (no checkbox) en la plantilla V7.
const CAMPOS_V7 = {
  fecha: [5, 4], solicitante: [6, 4], personaCargo: [7, 4], contacto: [8, 4],
  informeNo: [5, 19],
  ubicacion: [20, 16], marca: [21, 2], serie: [21, 16], modelo: [22, 2], codigo: [22, 16],
  diagnostico: [26, 1],
  observaciones: [50, 1],
  responsable: [60, 2], recibeSatisfaccion: [60, 19]
};

function marcarCasilla(grid, cambios, clave){
  const pos = CASILLAS_V7[clave];
  if(!pos) return;
  marcarCambio(grid, cambios, pos[0], pos[1], 'X');
}

function marcarCampo(grid, cambios, clave, valor, formato){
  if(valor == null || valor === '') return;
  const pos = CAMPOS_V7[clave];
  if(!pos) return;
  marcarCambio(grid, cambios, pos[0], pos[1], valor, formato);
}

// Para "Diagnóstico del equipo" y "Observaciones": la plantilla ya trae una frase
// por defecto ("Equipo programado para mantenimiento preventivo", "Equipo en buen
// estado físico y funcional."). Si el técnico escribe algo adicional, se agrega
// DEBAJO de esa frase (no la reemplaza) — así nunca se pierde el texto original.
function agregarCampoDebajo(grid, cambios, clave, valorAdicional){
  if(valorAdicional == null || valorAdicional === '') return;
  const pos = CAMPOS_V7[clave];
  if(!pos) return;
  const existente = (grid.filas[pos[0]] && grid.filas[pos[0]][pos[1]]) || '';
  const valorFinal = existente ? existente + '\n' + valorAdicional : valorAdicional;
  marcarCambio(grid, cambios, pos[0], pos[1], valorFinal);
}

// Para "Descripción de los procedimientos realizados" y ahora también para
// "Observaciones": si una línea de texto no cabe completa en el ancho de la fila, se
// corta ahí y sigue en la fila de abajo — igual que si se estuviera escribiendo a
// mano en las líneas impresas del formato.
function dividirEnFilas(texto, maxCaracteresPorFila){
  const lineasFinal = [];
  const parrafos = String(texto || '').split('\n');
  for(const parrafo of parrafos){
    const palabras = parrafo.trim().split(/\s+/).filter(Boolean);
    if(!palabras.length) continue; // renglón en blanco que haya dejado el técnico: se omite
    let lineaActual = '';
    for(const palabra of palabras){
      const propuesta = lineaActual ? lineaActual + ' ' + palabra : palabra;
      if(propuesta.length > maxCaracteresPorFila && lineaActual){
        lineasFinal.push(lineaActual);
        lineaActual = palabra;
      }else{
        lineaActual = propuesta;
      }
    }
    if(lineaActual) lineasFinal.push(lineaActual);
  }
  return lineasFinal;
}

// La fila fija de "Observaciones" (con "Equipo en buen estado físico y funcional.")
// nunca se toca. Lo que el técnico agregue se reparte en las filas que siguen,
// partiendo el texto por fila si hace falta — hay 5 filas disponibles después de la
// fija antes de llegar a la zona de firmas.
const FILAS_OBSERVACIONES_DISPONIBLES = 5;
const MAX_CARACTERES_FILA_OBSERVACIONES = 100;

function agregarObservacionesAdicionales(grid, cambios, textoAdicional, mantenerFija){
  if(textoAdicional == null || textoAdicional === '') return;
  const [filaFija, col] = CAMPOS_V7.observaciones;
  if(mantenerFija === false){
    // Se reemplaza por completo: el texto nuevo empieza desde la fila fija, y se
    // borra cualquier resto que hubiera quedado de la frase original en las filas
    // siguientes (por si el texto nuevo ocupa menos filas que antes).
    const lineas = dividirEnFilas(textoAdicional, MAX_CARACTERES_FILA_OBSERVACIONES)
      .slice(0, FILAS_OBSERVACIONES_DISPONIBLES + 1);
    for(let i = 0; i <= FILAS_OBSERVACIONES_DISPONIBLES; i++){
      marcarCambio(grid, cambios, filaFija + i, col, lineas[i] || '');
    }
    return;
  }
  const lineas = dividirEnFilas(textoAdicional, MAX_CARACTERES_FILA_OBSERVACIONES)
    .slice(0, FILAS_OBSERVACIONES_DISPONIBLES);
  lineas.forEach((linea, i) => marcarCambio(grid, cambios, filaFija + 1 + i, col, linea));
}
...
function calcularCambiosPlantilla(grid, equipo, cliente){
  ...
  marcarCampo(grid, cambios, 'fecha', cliente.fecha, IZQ_ABAJO);
  marcarCampo(grid, cambios, 'solicitante', cliente.solicitante);
  marcarCampo(grid, cambios, 'personaCargo', cliente.personaCargo);
  marcarCampo(grid, cambios, 'contacto', cliente.contacto, IZQ_ABAJO);
  marcarCampo(grid, cambios, 'informeNo', equipo.informeNo);

  marcarCampo(grid, cambios, 'ubicacion', equipo.ubicacion);
  marcarCampo(grid, cambios, 'marca', equipo.marca);
  marcarCampo(grid, cambios, 'serie', equipo.serie);
  marcarCampo(grid, cambios, 'modelo', equipo.modelo);
  marcarCampo(grid, cambios, 'codigo', equipo.codigo);
  agregarCampoDebajo(grid, cambios, 'diagnostico', equipo.diagnostico);

  if(CASILLAS_V7[equipo.tipoMantenimiento]) marcarCasilla(grid, cambios, equipo.tipoMantenimiento);
  if(equipo.claseFalla === 'Otro') marcarCasilla(grid, cambios, 'OtroFalla');
  else if(CASILLAS_V7[equipo.claseFalla]) marcarCasilla(grid, cambios, equipo.claseFalla);
  ...
  agregarObservacionesAdicionales(grid, cambios, equipo.observaciones, equipo._mantenerObservacionFija);
  marcarCampo(grid, cambios, 'responsable', equipo.responsable);
  marcarCampo(grid, cambios, 'recibeSatisfaccion', equipo.recibeSatisfaccion);
...
// A diferencia de "Reportes" (que solo se usa si ya existe), esta SIEMPRE deja el
// archivo dentro de la subcarpeta indicada — si no existe todavía dentro de la
// carpeta elegida, se crea sola, sin preguntar.
const cacheSubcarpetas = {};
// Toma el primer número que aparezca en el "Informe No." (por ejemplo, de "9015-26"
// toma 9015) para decidir si es par o impar — el guion y lo que venga después
// (como el año) no cuenta para esto. Si no hay ningún número, se asume impar.
function numeroReporteEsImpar(informeNo){
...
async function encontrarOCrearSubcarpeta(carpetaPadreId, nombreSubcarpeta){
  const clave = carpetaPadreId + '|' + nombreSubcarpeta;
  if(cacheSubcarpetas[clave]) return cacheSubcarpetas[clave];

  const q = "name = '" + nombreSubcarpeta + "' and mimeType = 'application/vnd.google-apps.folder' and '" + carpetaPadreId + "' in parents and trashed = false";
  const res = await fetch('https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(q) + '&fields=files(id,name)', {
    headers: driveHeaders()
  });
  if(!res.ok) throw new Error('No se pudo buscar la subcarpeta "' + nombreSubcarpeta + '" (HTTP ' + res.status + ')');
  const data = await res.json();
  const existente = (data.files || [])[0];
  if(existente){
    cacheSubcarpetas[clave] = existente.id;
    return existente.id;
  }
  ...
  if(!createRes.ok) throw new Error('No se pudo crear la subcarpeta "' + nombreSubcarpeta + '" (HTTP ' + createRes.status + ')');
  const creada = await createRes.json();
  cacheSubcarpetas[clave] = creada.id;
  return creada.id;
}

async function exportarComoPdf(sheetsFileId, sheetId, nombreResponsable){
  // Se usa el exportador directo de Sheets (no el de Drive) porque permite indicar con
  // "gid" cuál hoja exportar — así el PDF trae solo "PRIMERA", no ANEXO ni las otras
  // pestañas que también tiene el archivo. El tamaño carta y el ajuste a una página se
  // piden explícitos aquí (no basta con haberlo configurado en el Excel: al exportar,
  // Google no siempre respeta esa configuración guardada).
  ...
  const url = 'https://docs.google.com/spreadsheets/d/' + sheetsFileId + '/export?' + params.toString();
  const res = await fetch(url, { headers: driveHeaders() });
  if(!res.ok) throw new Error('No se pudo exportar a PDF (HTTP ' + res.status + ')');
  const bytesOriginales = await res.arrayBuffer();
  return await pegarLogoFirmaSello(bytesOriginales, nombreResponsable);
}

function base64ADatos(dataUri){
...
// Los dos técnicos que hacen el mantenimiento y sus firmas ya preparadas. Se
// reconocen por nombre (sin importar tildes/mayúsculas) contra lo que se haya
// escrito en "Responsable(s) ejecución"; si no coincide con ninguno, no se pega
// ninguna firma ahí (para no poner una firma que no corresponde).
const FIRMAS_TECNICOS = [
  { nombre: 'ramiro a. diaz', b64: () => FIRMA_RAMIRO_B64 },
  { nombre: 'ramiro diaz', b64: () => FIRMA_RAMIRO_B64 },
  { nombre: 'ramiro', b64: () => FIRMA_RAMIRO_B64 },
  { nombre: 'evelyn ramos', b64: () => FIRMA_EVELYN_B64 },
  { nombre: 'evelyn', b64: () => FIRMA_EVELYN_B64 }
];

function buscarFirmaTecnico(nombre){
  const clave = normNombre(nombre || '');
  if(!clave) return null;
  const encontrado = FIRMAS_TECNICOS.find(t => clave.includes(t.nombre) || t.nombre.includes(clave));
  return encontrado ? encontrado.b64() : null;
}

// La conversión de Excel a Google Sheets (necesaria para poder llenar los datos)
// termina dañando cómo se ven el logo, la firma y el sello, y a veces también el
// tamaño de la página — sin importar qué tan bien esté armado el Excel original. En
// vez de seguir peleando con esa conversión, se deja que Google solo se encargue del
// texto (que sí hace bien), se fuerza el tamaño carta directamente en el PDF, y el
// logo/firma/sello se pegan aparte, como imágenes ya preparadas, directo encima.
async function pegarLogoFirmaSello(bytesOriginalesPdf, nombreResponsable){
  const { PDFDocument } = PDFLib;
  const pdfDoc = await PDFDocument.load(bytesOriginalesPdf);
  const pagina = pdfDoc.getPages()[0];

  // Las posiciones de abajo están calculadas para una página carta perfecta
  // (612x792 puntos). Si Google entrega la página en otro tamaño, en vez de usar
  // page.scale() (que puede descuadrar dónde caen los dibujos agregados después),
  // se reescala cada posición a mano — así siempre coinciden, sin sorpresas.
  const ANCHO_CARTA = 612, ALTO_CARTA = 792; // 8.5x11" en puntos
  const { width: anchoActual, height: altoActual } = pagina.getSize();
  const sx = anchoActual / ANCHO_CARTA;
  const sy = altoActual / ALTO_CARTA;
  const pos = (x, y, w, h) => ({ x: x * sx, y: y * sy, width: w * sx, height: h * sy });

  const logoImg = await pdfDoc.embedJpg(base64ADatos(LOGO_INTELMEDICA_B64));
  pagina.drawImage(logoImg, pos(43, 737, 134, 34));

  const selloImg = await pdfDoc.embedPng(base64ADatos(SELLO_INTELMEDICA_B64));
  pagina.drawImage(selloImg, pos(208, 66, 88, 31));

  const firmaCoordinadorImg = await pdfDoc.embedPng(base64ADatos(FIRMA_RAMIRO_B64));
  pagina.drawImage(firmaCoordinadorImg, pos(310, 65, 62, 38));

  // Firma de quien ejecutó el mantenimiento, bajo "Responsable(s) ejecución" — misma
  // fila que la del coordinador, corrida a la columna de la izquierda (la distancia
  // entre ambas columnas se midió directo sobre un informe real).
  const firmaResponsableB64 = buscarFirmaTecnico(nombreResponsable);
  if(firmaResponsableB64){
    const firmaResponsableImg = await pdfDoc.embedPng(base64ADatos(firmaResponsableB64));
    pagina.drawImage(firmaResponsableImg, pos(131.6, 65, 62, 38));
  }
...
async function empujarEstadoADrive(){
  ...
  // Además de la sesión "actual" (para continuar en otro dispositivo), se guarda un
  // registro del último mantenimiento DE ESTE CLIENTE en particular — así, cuando se
  // vuelva a entrar a ese mismo cliente más adelante, se puede ofrecer continuar con
  // lo que faltó (informes o acta pendientes) en vez de empezar de cero.
  if(state.cliente.nombre && state.equipos.length){
    try{ await guardarUltimoMantenimientoCliente(state.cliente.nombre, state.equipos); }catch(e){ /* no bloquea el guardado principal */ }
  }
}

function nombreArchivoMantenimientoCliente(nombreCliente){
  return 'Ultimo_Mantenimiento_' + normNombre(nombreCliente).replace(/[^a-z0-9]+/g, '_') + '.json';
}
...
async function guardarUltimoMantenimientoCliente(nombreCliente, equipos){
  ...
  if(existente){
    ...
    const metadata = { name: nombreArchivo, parents: [CONFIG.MANTENIMIENTOS_FOLDER_ID] };
    const form = new FormData();
    form.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));
...
// Devuelve el último mantenimiento guardado para este cliente (o null si nunca se ha
// guardado uno, o si no hay Drive conectado).
async function cargarUltimoMantenimientoCliente(nombreCliente){
  ...
  if(!archivo) return null;
  const contenidoRes = await fetch('https://www.googleapis.com/drive/v3/files/' + archivo.id + '?alt=media', { headers: driveHeaders() });
  if(!contenidoRes.ok) return null;
  try{ return await contenidoRes.json(); }catch(e){ return null; }
}

// Al entrar a un cliente que ya tiene un mantenimiento guardado, se pregunta si se
// quiere seguir con ese (para completar lo que falte: informes o acta pendientes) o
// si es una visita nueva. Si se elige continuar, se traen los equipos de esa vez —
// cada uno conserva la insignia de "Ya generado" según cómo haya quedado entonces.
async function ofrecerContinuarUltimoMantenimiento(nombreCliente){
  const ultimo = await cargarUltimoMantenimientoCliente(nombreCliente);
  if(!ultimo || !ultimo.equipos || !ultimo.equipos.length) return;
  const continuar = confirm(
    'Ya hay un mantenimiento guardado para "' + nombreCliente + '" (' + ultimo.equipos.length + ' equipo(s)).\n\n' +
    'Aceptar = continuar ese mismo mantenimiento (para completar informes o el acta que hayan quedado pendientes).\n' +
    'Cancelar = es una visita nueva (empieza sin esos equipos).'
  );
  if(continuar){
    state.equipos = ultimo.equipos;
    state.draft = null;
    state.editingId = null;
    saveState();
    render();
    showToast('Se cargó el último mantenimiento de este cliente.');
  }
}
...
async function revisarSincronizacionRemota(){
  ...
  try{
    ...
    // Si hay una visita en curso más reciente (de este dispositivo o de otro), se
    // pregunta al entrar si se quiere continuar con esos equipos o empezar una
    // visita nueva — la pantalla ya arrancó en blanco, así que "no" simplemente la
    // deja así.
    if(remotoTieneAlgo && remotoTs > localTs + 5000){
      const cantidad = (remoto.equipos && remoto.equipos.length) || 0;
      const continuar = confirm(
        'Hay una visita en curso para "' + (remoto.cliente && remoto.cliente.nombre ? remoto.cliente.nombre : 'un cliente') + '" (' + cantidad + ' equipo(s)).\n\n' +
        'Aceptar = continuar con el ingreso de esos equipos.\n' +
        'Cancelar = iniciar una visita nueva.'
      );
      if(continuar){
        ...
        saveState();
        goTo('cliente');
        showToast('Se continuó la visita en curso.');
      }
    }
...
async function prepararGuardadoMasivo(){
  if(!state.driveToken){ showToast('Primero conecta tu cuenta de Google Drive.'); return; }
  const pendientes = equiposPendientes();
  if(!pendientes.length){ showToast('No quedan equipos pendientes por generar.'); return; }
  const sinNumero = pendientes.filter(eq => !eq.informeNo);
  if(sinNumero.length){
    if(!confirm(sinNumero.length + ' de ' + pendientes.length + ' equipo(s) no tienen "Informe No." — esos van a quedar sin ese número. ¿Continuar de todas formas?')) return;
  }
  for(const eq of pendientes){
    if(eq.observaciones){
      eq._mantenerObservacionFija = confirm(
        eq.tipo + ': ¿la observación "Equipo en buen estado físico y funcional." se debe mantener además de lo que escribiste?\n\n' +
        'Aceptar = se mantiene esa frase y se agrega lo tuyo debajo (equipo quedó bien).\n' +
        'Cancelar = se reemplaza por completo con lo que escribiste (equipo NO quedó listo).'
      );
    }
  }
  confirmacionCarpeta[ID_MASIVO] = { cargando: true, opciones: null, elegida: null, buscando: false, filtro: '', navPila: null };
  render();
  try{
    const folderId = await encontrarOCrearCarpetaCliente(state.cliente.nombre);
    const nombre = await obtenerNombreCarpeta(folderId);
    confirmacionCarpeta[ID_MASIVO] = { cargando: false, opciones: null, elegida: { id: folderId, nombre }, buscando: false, filtro: '', navPila: null };
  }catch(err){
    confirmacionCarpeta[ID_MASIVO] = { cargando: false, opciones: null, elegida: null, buscando: false, filtro: '', navPila: null, error: err.message };
  }
  render();
}

async function confirmarGuardadoMasivo(){
  const est = confirmacionCarpeta[ID_MASIVO];
  if(!est || !est.elegida) return;
  const folderId = est.elegida.id;
  delete confirmacionCarpeta[ID_MASIVO];
  render();

  const setStatus = (msg, cls) => {
    const el2 = document.getElementById('status_' + ID_MASIVO);
    if(el2){ el2.textContent = msg; el2.className = 'status-line ' + (cls||''); }
  };

  const pendientes = equiposPendientes();
  let ok = 0, fallidos = [];
  for(let i = 0; i < pendientes.length; i++){
    const eq = pendientes[i];
    setStatus(`Generando ${i+1} de ${pendientes.length}: ${eq.tipo}…`);
    try{
      await generarInformeOficial(eq.id, folderId);
      if(eq.informeOficialGenerado) ok++;
      else fallidos.push(eq.tipo);
    }catch(err){
      fallidos.push(eq.tipo);
    }
  }

  render();
  if(fallidos.length){
    setStatus(`✓ ${ok} generado(s). ✗ ${fallidos.length} con error: ${fallidos.join(', ')}`, 'err');
  }else{
    setStatus(`✓ Los ${ok} informe(s) pendientes se generaron y guardaron en Drive.`, 'ok');
  }
  showToast('Generación masiva terminada.');
}
...
async function prepararGuardado(id){
  ...
  if(eq && eq.observaciones){
    eq._mantenerObservacionFija = confirm(
      '¿La observación "Equipo en buen estado físico y funcional." se debe mantener además de lo que escribiste?\n\n' +
      'Aceptar = se mantiene esa frase y se agrega lo tuyo debajo (equipo quedó bien).\n' +
      'Cancelar = se reemplaza por completo con lo que escribiste (equipo NO quedó listo).'
    );
  }
...
function panelConfirmacionHtml(eq, esMasivo){
  ...
  return `
    <div class="card" style="background:#f4f7fb;">
      <div class="card-title">📁 ¿Dónde se guarda${esMasivo ? ' — todos los pendientes' : ''}?</div>
      <div class="status-line" style="text-align:left;margin:0 0 10px;">Se guardará en: <b>${escapeHtml(est.elegida.nombre)}</b></div>
      ${listaHtml}
      ${est.buscando ? '' : `
      <button class="btn btn-primary btn-sm" style="width:100%;" data-action="${esMasivo ? 'confirmarGuardadoMasivo' : 'confirmarGuardado'}" data-id="${eq.id}">Guardar aquí</button>
...
async function generarInformeOficial(id, folderId){
  ...
  try{
    ...
    const pdfBlob = await exportarComoPdf(sheetFile.id, grid.sheetId, eq.responsable);
    ...
    eq.informeOficialGenerado = true;
    eq.informeOficialFecha = todayISO();
    saveState();
    render();
...
async function manejarNombreClienteIngresado(nombre){
  ...
  try{
    ...
    if(existente){
      ...
      if(!state.equipos.length){
        await ofrecerContinuarUltimoMantenimiento(nombre);
      }
    }else{
...
function wireCommonHandlers(){
  ...
  // Sugerencias con lo ya escrito antes en estos campos
  app.querySelectorAll('input[data-historial]').forEach(input => {
    input.addEventListener('input', () => renderHistorialSugerencias(input));
    input.addEventListener('focus', () => renderHistorialSugerencias(input));
    input.addEventListener('blur', () => {
      guardarEnHistorial(input.dataset.path, input.value);
      // se espera un momento para que alcance a registrar el toque sobre una sugerencia
      setTimeout(() => {
        const box = document.getElementById('hist_' + input.id);
        if(box && document.activeElement !== input){ box.innerHTML = ''; box.className = ''; }
      }, 250);
    });
  });
  ...
  if(nombreClienteInput){
    let nombreAlEnfocar = nombreClienteInput.value;
    nombreClienteInput.addEventListener('focus', () => {
      nombreAlEnfocar = nombreClienteInput.value;
      if(nombreClienteInput.value) renderClienteSuggestions(nombreClienteInput.value);
    });
    nombreClienteInput.addEventListener('input', () => {
      renderClienteSuggestions(nombreClienteInput.value);
    });
    nombreClienteInput.addEventListener('blur', () => {
      if(!confirmarCambioClienteSiHaceFalta(nombreAlEnfocar, nombreClienteInput.value)){
        state.cliente.nombre = nombreAlEnfocar; // se canceló: se deja el nombre como estaba
        saveState();
        render();
        return;
      }
...
function onAppClick(e){
  ...
  switch(action){
    ...
    case 'iniciarOtroCliente': iniciarOtroCliente(); break;
    ...
    case 'pickClienteExistente': pickClienteExistente(value); break;
    case 'pickHistorial': pickHistorial(target.dataset.target, value); break;
    ...
    case 'prepararGuardadoMasivo': prepararGuardadoMasivo(); break;
    case 'pedirSaludoActa': pedirSaludoActa(); break;
    case 'cancelarActaEntrega': cancelarActaEntrega(); break;
    case 'generarActaEntrega': generarActaEntrega(target.dataset.saludo); break;
    case 'confirmarGuardadoMasivo': confirmarGuardadoMasivo(); break;
