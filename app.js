...
async function pegarLogoFirmaSello(bytesOriginalesPdf){
  ...
  if(Math.abs(anchoActual - ANCHO_CARTA) > 5 || Math.abs(altoActual - ALTO_CARTA) > 5){
    pagina.scale(ANCHO_CARTA / anchoActual, ALTO_CARTA / altoActual);
  }

  const logoImg = await pdfDoc.embedJpg(base64ADatos(LOGO_INTELMEDICA_B64));
  pagina.drawImage(logoImg, { x: 65, y: 709, width: 118, height: 24 });

  const selloImg = await pdfDoc.embedPng(base64ADatos(SELLO_INTELMEDICA_B64));
  pagina.drawImage(selloImg, { x: 208, y: 66, width: 88, height: 31 });

  const firmaImg = await pdfDoc.embedPng(base64ADatos(FIRMA_RAMIRO_B64));
  pagina.drawImage(firmaImg, { x: 310, y: 65, width: 62, height: 38 });
