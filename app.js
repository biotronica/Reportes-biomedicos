...
async function obtenerGridPrimera(spreadsheetId){
  const url = 'https://sheets.googleapis.com/v4/spreadsheets/' + spreadsheetId
    + '?includeGridData=true&ranges=PRIMERA&fields=sheets(properties(sheetId,title),data(rowData(values(formattedValue))),merges)';
  const res = await fetch(url, { headers: driveHeaders() });
  if(!res.ok) throw new Error('No se pudo leer la hoja PRIMERA (HTTP ' + res.status + ')');
  const data = await res.json();
  const hoja = (data.sheets || [])[0];
  if(!hoja) throw new Error('La plantilla no tiene una hoja llamada "PRIMERA".');
  const filas = ((hoja.data || [])[0] && hoja.data[0].rowData) || [];
  const tabla = filas.map(fila => (fila.values || []).map(v => v.formattedValue || ''));
...
function marcarSiNoEnFila(grid, cambios, labelText, siValor){
  ...
  for(const rCandidata of [label.r, label.r + 1]){
    ...
    (grid.filas[rCandidata] || []).forEach((val, c) => {
