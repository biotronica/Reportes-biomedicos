...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: '',
  // Carpeta MANTENIMIENTOS donde vive "Formatos Mantenimientos Preventivos" y las
  // carpetas de cada cliente. Los informes oficiales se guardan directo en la carpeta
  // del cliente correspondiente (se busca por nombre, o se crea si no existe).
...
// (sin importar mayúsculas/minúsculas ni espacios de más, ya que tus carpetas existentes
// no siguen un mismo formato — "UCIS DE COLOMBIA", "Clinica CES", etc.). Si no existe
// ninguna con ese nombre, crea una nueva para ese cliente.
async function encontrarOCrearCarpetaCliente(nombreCliente){
  ...
  const exacta = (data.files || []).find(f => (f.name || '').trim().toLowerCase() === objetivo);
