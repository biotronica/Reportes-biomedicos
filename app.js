...
function writeCell(sheet, r, c, value){
  ...
  // Ampliar el rango usado de la hoja si escribimos fuera de sus límites actuales
...
// Escribe el valor en la celda inmediatamente a la derecha de la primera celda que
// contenga labelText (coincidencia exacta, sin importar espacios extra).
function setRightOf(sheet, labelText, value, offset){
  const matches = findCellsWithText(sheet, labelText);
  if(!matches.length) return false;
  const { r, c } = XLSX.utils.decode_cell(matches[0]);
  writeCell(sheet, r, c + (offset || 1), value);
  return true;
}
