...
async function exportarComoPdf(sheetsFileId, sheetId){
  ...
  return await pegarLogoFirmaSello(bytesOriginales);
}
...
async function pegarLogoFirmaSello(bytesOriginalesPdf){
  ...
  const firmaImg = await pdfDoc.embedPng(base64ADatos(FIRMA_RAMIRO_B64));
  pagina.drawImage(firmaImg, pos(310, 65, 62, 38));
...
async function generarInformeOficial(id, folderId){
  ...
  try{
    ...
    const pdfBlob = await exportarComoPdf(sheetFile.id, grid.sheetId);
